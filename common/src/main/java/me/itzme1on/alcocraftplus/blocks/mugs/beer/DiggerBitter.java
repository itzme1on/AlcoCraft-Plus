...
public class DiggerBitter extends MugBlock {
    public DiggerBitter(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick(@NotNull BlockState blockState, @NotNull Level level, @NotNull BlockPos blockPos, Random random) {
        if (random.nextFloat() < 0.4f) {
            double d0 = (double)blockPos.getX() + 0.4D + random.nextDouble() * 0.2D;
            double d1 = (double)blockPos.getY() + 0.2D + random.nextDouble() * (0.2D - 0.02D);
            double d2 = (double)blockPos.getZ() + 0.4D + random.nextDouble() * 0.2D;
