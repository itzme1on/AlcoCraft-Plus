...
public class IceBeer extends MugBlock {
    public IceBeer(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick(@NotNull BlockState blockState, @NotNull Level level, @NotNull BlockPos blockPos, Random random) {
        if (random.nextFloat() < 0.4f) {
            ...
            level.addParticle(ParticleTypes.SNOWFLAKE, d0, d1, d2, 0.0D, random.nextDouble() * 0.1D, 0.0D);
        }
    }
}
