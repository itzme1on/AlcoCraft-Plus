...
public class AttractEffect extends MobEffect {
    ...
    private static final DustParticleOptions DUST_COLOR = new DustParticleOptions(
            new Vector3f(170f / 255f, 14f / 255f, 1f / 255f),
            0.5f
    );
    ...
    private static MobEffect attractEffect() {
        if (ATTRACT_EFFECT == null) {
            ATTRACT_EFFECT = EffectsRegistry.ATTRACT.get();
        }
    ...
    private static LivingEntity findNearest(List<LivingEntity> holders, ItemEntity item) {
        ...
        for (LivingEntity holder : holders) {
            double dist2 = squaredDistanceFeetToItem(holder, item);
    ...
    private static Vec3 entityFeet(LivingEntity entity) {
        return new Vec3(entity.getX(), entity.getY() + 0.05D, entity.getZ());
    }
    ...
    public void applyEffectTick(LivingEntity entity, int amplifier) {
        ...
        AABB searchBox = entity.getBoundingBox().inflate(SEARCH_RADIUS_BLOCKS);
        ...
        List<LivingEntity> attractEffectHolders = level.getEntitiesOfClass(LivingEntity.class, searchBox)
                .stream()
                .filter(e -> e.isAlive() && e.hasEffect(attractEffect()))
                .toList();

        if (attractEffectHolders.isEmpty() || nearbyItems.isEmpty()) return;
        ...
        for (LivingEntity holder : attractEffectHolders) {
            MobEffectInstance instance = holder.getEffect(attractEffect());
            amplifierByHolderId.put(holder.getId(), instance == null ? 0 : instance.getAmplifier());
        }
