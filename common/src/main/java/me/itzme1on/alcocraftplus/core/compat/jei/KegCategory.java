...
public class KegCategory implements IRecipeCategory<KegRecipes> {
    ...
    public @NotNull Component getTitle() {
        return Component.translatable("item.alcocraftplus.keg");
    }
    ...
    @Override
    public void draw(KegRecipes recipe, IRecipeSlotsView recipeSlotsView, PoseStack poseStack, double mouseX, double mouseY) {
        arrow.draw(poseStack, 113, SLOTS_Y_POSITION - 1);
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, KegRecipes recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.OUTPUT, 148, SLOTS_Y_POSITION).addItemStack(recipe.getResultItem());
    }
}