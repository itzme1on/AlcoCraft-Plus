...
public class KegGuiHandler extends AbstractContainerMenu {
    ...
    private boolean isIngredient(ItemStack stack) {
        var list = this.level.getRecipeManager().getAllRecipesFor(RecipesRegistry.KEG_RECIPE_TYPE.get());

        if (list.isEmpty()) return true;

        return list.stream().anyMatch(holder -> holder.value().getIngredients().stream().anyMatch(ingredient -> ingredient.test(stack)));
    }
